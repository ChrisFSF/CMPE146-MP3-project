...
uint16_t adc__get_channel_reading_with_burst_mode(adc_channel_e channel_num) {
  uint16_t result = 0;
  const uint16_t twelve_bits = 0x0FFF; // result store in GDR reg 15:4
  if ((ADC__CHANNEL_2 == channel_num) || (ADC__CHANNEL_4 == channel_num) || (ADC__CHANNEL_5 == channel_num)) {
...
void clear_all_channels() { LPC_ADC->CR &= ~(0xFF << 0); }

void set_channel(adc_channel_e channel_num) {

  LPC_ADC->CR |= (1 << channel_num);
  LPC_ADC->CR &= ~(7 << 24);
}

void set_burst_mode(bool status) {
  if (status) {
    LPC_ADC->CR |= (1 << 16);
#include "adc.h"
#include <stdint.h>
