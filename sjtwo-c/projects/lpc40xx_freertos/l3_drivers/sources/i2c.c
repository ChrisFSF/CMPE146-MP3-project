...
void i2c2__slave_init(uint8_t slave_address_to_respond_to) {
  ...
  LPC_I2C2->ADR0 = (slave_address_to_respond_to & 0xFE);
  // LPC_I2C2->MASK0 = 0x30; // default is 0x0011 0000
}
...
static bool i2c__handle_state_machine(i2c_s *i2c) {
  ...
  case I2C__STATE_SR_MASTER_DATA_ACK: // 0x80:
    if (i2c->slave_count == 0) {
      i2c->slave_reg_addr = lpc_i2c->DAT;
      ...
      if (i2c_slave_callback__write_memory(i2c->slave_reg_addr++, value)) {
        I2C__DEBUG_PRINTF("  HW State: 0x%02X: successed to write", i2c_state);
        i2c__set_ack_flag(lpc_i2c);
      } else {
  ...
  case I2C__STATE_ST_MASTER_ADDR_ACK: // 0xA8
    i2c->slave_reg_addr = i2c->slave_starting_reg_addr;
    if (i2c_slave_callback__read_memory(i2c->slave_reg_addr++, &i2c->slave_data_buffer)) {
      I2C__DEBUG_PRINTF("  HW State: 0x%02X: successed to read", i2c_state);
