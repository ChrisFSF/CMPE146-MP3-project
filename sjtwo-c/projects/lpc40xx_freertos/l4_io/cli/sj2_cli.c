...
void sj2_cli__init(void) {
  ...
  static app_cli__command_s taskctl = {.command_name = "taskctl",
                                       .help_message_for_command = "taskctl suspend/resume taskname\n",
                                       .app_cli_handler = cli__task_control}; // personal func
  static app_cli__command_s scan_dir = {
      .command_name = "scan", .help_message_for_command = "scan directory.\n", .app_cli_handler = cli__scan_dir};

  static app_cli__command_s play = {
      .command_name = "play", .help_message_for_command = "play song.mp3\n", .app_cli_handler = cli__playsong};
  // Add your CLI commands in descending sorted order
  app_cli__add_command_handler(&sj2_cli_struct, &play);
  app_cli__add_command_handler(&sj2_cli_struct, &scan_dir);
  app_cli__add_command_handler(&sj2_cli_struct, &taskctl); // personal func
